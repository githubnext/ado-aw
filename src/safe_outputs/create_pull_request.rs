...
/// Resolve the target repository alias and ADO repository ID for `repository`
/// against the pipeline's allowed-repository list.
///
/// Returns `Ok(Err(result))` for a user-facing failure (e.g. repository not
/// allowed); `Ok(Ok((alias, repo_id)))` on success.
...
/// Result of loading and validating the patch file: the resolved path, its
/// content, and the `--exclude` flags derived from `excluded-files` config.
struct PatchValidation {
    patch_path: std::path::PathBuf,
    exclude_args: Vec<String>,
}

/// Load the patch file, verify its size/integrity/security constraints, and
/// compute the `--exclude` flags for excluded-files patterns.
///
/// Returns `Ok(Err(result))` for a user-facing failure; `Ok(Ok(validation))`
/// on success.
async fn load_and_validate_patch(
...
) -> anyhow::Result<Result<PatchValidation, ExecutionResult>> {
    ...
    Ok(Ok(PatchValidation {
        patch_path,
        exclude_args,
    }))
}

/// Verify `repo_git_dir` is a git repository and create a worktree at
/// `target_branch` inside a fresh temp directory.
///
/// Returns `Ok(Err(result))` for a user-facing failure; on success returns
/// the owning [`tempfile::TempDir`] (keep alive for the worktree's lifetime),
/// the worktree path, and a [`WorktreeGuard`] that removes the worktree on drop.
async fn create_target_worktree(
    repo_git_dir: &std::path::Path,
    target_branch: &str,
) -> anyhow::Result<Result<(tempfile::TempDir, std::path::PathBuf, WorktreeGuard), ExecutionResult>>
{
    ...
    Ok(Ok((temp_dir, worktree_path, worktree_guard)))
}

/// Resolve the base commit to push against: prefer the merge-base SHA
/// recorded at patch generation time (Stage 1); fall back to querying the
/// ADO refs API for the target branch's current tip.
///
/// Returns `Ok(Err(result))` for a user-facing failure; `Ok(Ok(sha))` on success.
async fn resolve_base_commit(
    recorded_base_commit: &Option<String>,
    client: &reqwest::Client,
    refs_url: &str,
    token: &str,
...
/// Check whether `source_branch` already exists in ADO (e.g. from a retry or
/// previous run) and, if so, rename it with a fresh random suffix. Retries up
/// to 3 times. Returns the (possibly renamed) branch name and its full ref.
...
impl Executor for CreatePrResult {
    ...
    async fn execute_impl(&self, ctx: &ExecutionContext) -> anyhow::Result<ExecutionResult> {
        ...
        // Validate and load the patch file
        let PatchValidation {
            patch_path,
            exclude_args,
            ..
        ...
        let (_temp_dir, worktree_path, _worktree_guard) =
        ...
        // NDJSON entries).
        let base_commit =
            match resolve_base_commit(&self.base_commit, &client, &refs_url, token).await? {
                Ok(sha) => sha,
                Err(result) => return Ok(result),
            };
        ...
        // and, if so, rename it with a fresh random suffix.
        (source_branch, source_ref) =
            resolve_unique_source_branch(&client, org_url, project, &repo_id, token, source_branch)
                .await?;
