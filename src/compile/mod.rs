...
async fn compile_pipeline_inner(
    ...
    for warning in common::repository_write_readiness_warnings(&front_matter) {
...
/// Warn on stderr if `pipeline_path`'s header records a compiler version
/// different from the one currently running.
fn warn_on_version_mismatch(header_meta: &crate::detect::HeaderMetadata) {
    let current_version = env!("CARGO_PKG_VERSION");
...
/// Resolve the absolute source markdown path recorded in a compiled
/// pipeline's `@ado-aw` header, relative to the enclosing git repository
/// root (walked up from `pipeline_path`).
fn resolve_header_source_path(
    pipeline_path: &Path,
    header_meta: &crate::detect::HeaderMetadata,
) -> Result<PathBuf> {
    ...
    Ok(repo_root.join(&header_meta.source))
}

/// Compile `source_path` fresh into pipeline YAML, using the same
/// import-resolution, codemod, and repo-resolution steps as `compile_markdown`,
/// so `check` validates the exact same fully-merged pipeline that `compile`
/// would produce.
async fn compile_fresh_from_header_source(
    source_path: &Path,
    pipeline_path: &Path,
    header_meta: &crate::detect::HeaderMetadata,
    ...
    Ok((pipeline_yaml, front_matter.name.clone()))
}
...
pub async fn check_pipeline(pipeline_path: &str) -> Result<()> {
    ...
    warn_on_version_mismatch(&header_meta);

    // The header stores the source path relative to the repository root.
    let source_path = resolve_header_source_path(pipeline_path, &header_meta)?;
    ...
    let (pipeline_yaml, pipeline_name) =
        compile_fresh_from_header_source(&source_path, pipeline_path, &header_meta).await?;
