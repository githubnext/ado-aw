...
///
/// Currently unused now that the prompt path is pure bash + awk and
/// doesn't reference a JS bundle. Kept as a reusable helper for any
/// future bundle consumer.
#[allow(dead_code)]
...
///
/// Today only the gate-evaluator path consumes the bundle; future
/// helpers (e.g. a queue poller) can join the same dedupe by
/// returning `true` from [`CompilerExtension::needs_scripts_bundle`].
