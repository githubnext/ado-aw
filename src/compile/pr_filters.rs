...
mod tests {
    use crate::compile::common::{
        generate_agentic_depends_on, generate_pr_trigger, generate_setup_job,
    };
    ...
    fn test_generate_pr_trigger_with_filters_only_no_pr_block() {
        ...
        assert!(
            result.is_empty(),
            "filters-only should not emit a pr: block (use default trigger)"
        );
    }
    ...
    fn test_generate_setup_job_with_filters_no_extension_creates_empty() {
        ...
        assert!(
            result.is_empty(),
            "filters without extension should produce empty setup job"
        );
    }
    ...
    fn test_generate_setup_job_with_user_steps_and_filters() {
        ...
        let result =
            generate_setup_job(&[step], "MyPool", Some(&filters), None, &[], &ctx).unwrap();
        ...
        assert!(
            ...
            "user steps should reference gate output"
        );
    }
    ...
    fn test_generate_setup_job_without_filters_unchanged() {
        ...
        assert!(
            result.is_empty(),
            "no setup steps and no filters should produce empty string"
        );
    }
    ...
    fn test_generate_agentic_depends_on_with_pr_filters() {
        ...
        assert!(
            result.contains("dependsOn: Setup"),
            "should depend on Setup"
        );
    ...
    fn test_generate_setup_job_gate_spec_via_extension() {
        ...
        let filters = PrFilters {
            ...
            source_branch: Some(PatternFilter {
                pattern: "feature/*".into(),
            }),
            target_branch: Some(PatternFilter {
                pattern: "main".into(),
            }),
            ..Default::default()
        };
    ...
    fn test_agentic_depends_on_with_expression() {
        ...
        assert!(
            !result.contains("dependsOn"),
            "no dependsOn without setup/filters"
        );
        assert!(result.contains("condition:"), "should have condition");
        assert!(
            result.contains("Custom.ShouldRun"),
            "should include expression"
        );
        assert!(
            result.contains("succeeded()"),
            "should still require succeeded"
        );
    }
