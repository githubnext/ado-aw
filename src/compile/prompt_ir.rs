...
//! (`prompt.js`). Mirrors the design of [`super::filter_ir::GateSpec`].
//!
...
//! `prompt.js` step. At pipeline runtime, `prompt.js` decodes the spec,
//! validates the version, reads the source markdown from the workspace,
//! strips its front matter, applies single-pass variable substitution,
//! appends supplements, and writes the rendered prompt to `output_path`.
//!
//! The JSON Schema generated by [`generate_prompt_spec_schema`] is the
//! contract surface between Rust and the TypeScript runtime bundle:
//! `npm run codegen` in `scripts/ado-script/` converts it to
//! `src/shared/types-prompt.gen.ts` via `json-schema-to-typescript`.
...
pub fn generate_prompt_spec_schema() -> String {
    ...
    serde_json::to_string_pretty(&schema).expect("PromptSpec schema must serialize")
}
...
mod tests {
    ...
    #[test]
    fn supplement_serializes_with_camel_or_snake_keys() {
        let supp = PromptSupplement {
            name: "Lean 4".into(),
            content: "Use lake build for verification.".into(),
        };
        let json = serde_json::to_string(&supp).unwrap();
        assert!(json.contains("\"name\":\"Lean 4\""));
        assert!(json.contains("\"content\":\"Use lake build for verification.\""));
    }
}
