...
fn incomplete_provenance_finding(report: &CustomSafeOutputJobAudit) -> Option<Finding> {
    ...
    Some(Finding {
        category: String::from("safe_outputs"),
        severity: Severity::High,
        title: format!("Incomplete custom component provenance for {}", report.tool),
        description: format!(
...
fn ran_without_approval_finding(
    ...
    Some(Finding {
        ...
        severity: Severity::High,
        title: format!(
            "Custom reviewed job ran without approval for {}",
            report.tool
        ),
        description: format!(
...
fn matching_timeline_jobs<'a>(
    ...
    let id_name_matches = jobs
        .iter()
        .filter(|job| {
            expected_job_id.is_some_and(|expected| candidate_matches_job_id(&job.name, expected))
