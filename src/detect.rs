...
/// Metadata extracted from an agentic workflow header comment.
#[derive(Debug, Clone)]
...
/// Scan a directory for agentic workflow YAML files.
///
...
pub async fn detect_pipelines(root: &Path) -> Result<Vec<DetectedPipeline>> {
    info!("Scanning for agentic workflows in: {}", root.display());
    let mut results = Vec::new();
    scan_directory(root, root, &mut results).await?;
    info!("Found {} agentic workflow(s)", results.len());
    Ok(results)
}
...
fn scan_directory<'a>(
    ...
    Box::pin(async move {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            {
                debug!("Detected agentic workflow: {}", path.display());
...
/// Check if a YAML file is an agentic workflow and extract its metadata.
///
...
";
    ...
    async fn test_detect_pipelines_in_temp_dir() {
        ...
        // Write an agentic workflow YAML
