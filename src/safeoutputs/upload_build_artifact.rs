...
//!
//! Lets an agent propose attaching a workspace file to an existing (typically
//! already-finished) Azure DevOps build via the **build attachments** REST API
//! (`PUT /_apis/build/builds/{buildId}/attachments/{type}/{name}`). Unlike
//! `upload-artifact` (which emits a `##vso[artifact.upload]` logging command
//! tied to the *current* run), this tool can attach a file to **any** build
//! the executor's token has access to — useful for posthumously decorating a
//! build with a generated report, screenshot, or log bundle.
//!
//! The flow mirrors `upload-artifact` and `create-pull-request`:
//!
...
//!   `allowed-build-ids`, `name-prefix`, `attachment-type`), and PUTs the
...
/// Parameters for attaching a workspace file to an existing ADO build.
#[derive(Deserialize, JsonSchema)]
pub struct UploadBuildArtifactParams {
    /// The build ID to attach the file to. Must be positive. The build does
    /// not need to be running — this tool is designed to attach files
    /// posthumously to already-finished builds.
    pub build_id: i64,
...
tool_result! {
    ...
    default_max = 3,
    /// Result of attaching a workspace file to an existing ADO build.
    pub struct UploadBuildArtifactResult {
        /// Build ID the file should be attached to.
        build_id: i64,
...
pub struct UploadBuildArtifactConfig {
    ...
    /// build ID accessible to the executor's token is allowed.
...
impl Executor for UploadBuildArtifactResult {
    fn dry_run_summary(&self) -> String {
        format!(
            ...
            self.file_path, self.artifact_name, self.build_id
        )
    }
    ...
    async fn execute_impl(&self, ctx: &ExecutionContext) -> anyhow::Result<ExecutionResult> {
        ...
        debug!(
            ...
            self.build_id, self.artifact_name, self.file_path
        );
        ...
        // Check build-id allow-list (if configured).
        if !config.allowed_build_ids.is_empty()
            && !config.allowed_build_ids.contains(&self.build_id)
        {
        ...
        if ctx.dry_run {
            return Ok(ExecutionResult::success(format!(
                ...
                self.file_path, file_size, final_name, self.build_id
            )));
        }
        ...
        info!(
            ...
            file_size, self.build_id, attachment_type, final_name
        );
        let response = client
            .put(&url)
        ...
        if response.status().is_success() {
            ...
            info!(
                ...
                self.file_path, self.build_id, final_name
            );
            ...
            Ok(ExecutionResult::failure(format!(
                ...
                self.build_id, status, error_body
            )))
        }
    }
}
...
mod tests {
    ...
    fn make_params(build_id: i64, artifact_name: &str, file_path: &str) -> UploadBuildArtifactParams {
    ...
    fn test_params_deserializes() {
        let json = r#"{"build_id": 1234, "artifact_name": "agent-report", "file_path": "out/report.pdf"}"#;
        let params: UploadBuildArtifactParams = serde_json::from_str(json).unwrap();
        assert_eq!(params.build_id, 1234);
    ...
    fn test_params_validate_accepts_valid() {
        assert!(make_params(1, "agent-report", "out/report.pdf").validate().is_ok());
    }

    #[test]
    fn test_validation_rejects_zero_build_id() {
        assert!(make_params(0, "agent-report", "out/report.pdf").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_negative_build_id() {
        assert!(make_params(-1, "agent-report", "out/report.pdf").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_empty_artifact_name() {
        assert!(make_params(1, "", "out/report.pdf").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_artifact_name_starting_with_dot() {
        assert!(make_params(1, ".hidden", "out/report.pdf").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_artifact_name_with_space() {
        assert!(make_params(1, "my artifact", "out/report.pdf").validate().is_err());
    }
    ...
    fn test_validation_accepts_dotted_artifact_name() {
        assert!(make_params(1, "agent.report.v2", "out/report.pdf").validate().is_ok());
    }

    #[test]
    fn test_validation_rejects_empty_file_path() {
        assert!(make_params(1, "agent-report", "").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_path_traversal() {
        assert!(make_params(1, "agent-report", "../etc/passwd").validate().is_err());
    }
    ...
    fn test_validation_rejects_backslash_traversal() {
        assert!(make_params(1, "agent-report", "src\\..\\secret.txt").validate().is_err());
    }

    #[test]
    fn test_validation_rejects_dotgit_component() {
        assert!(make_params(1, "agent-report", ".git/config").validate().is_err());
    }
