...
builds the exact same five workflows with a compiler from the current PR or
nightly `main`, stages the regenerated sources/YAML on a short-lived
`ado-aw-mirror` ref, and runs five fixed candidate definitions. Keeping both
lanes distinguishes release packaging/download failures from regressions in
unreleased compiler output.
