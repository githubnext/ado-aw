...
The candidate lane is deliberately ephemeral. It must not commit any generated
lock back to the GitHub branch; the checked-in `tests/safe-outputs/` locks
remain generated by the latest released compiler so their runtime integrity
check and downloaded binary cannot drift.
