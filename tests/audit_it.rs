...
async fn audit_pipeline_artifact_layouts_are_equivalent_end_to_end() {
    ...
    let step_trace: Value = serde_json::from_slice(&step_trace.stdout).expect("trace --step JSON");
    ...
    let missing_step_trace: Value =
        serde_json::from_slice(&missing_step_trace.stdout).expect("trace --step (missing) JSON");
    assert!(
