...
fn find_gate_spec(value: &Value) -> Option<String> {
    match value {
        Value::Mapping(mapping) => {
            ...
            if script.is_some_and(|script| script.contains("node '/tmp/ado-aw-scripts/ado-script/gate.js'")) {
...
fn run_gate(gate_js: &Path, gate_spec: &str, pr_title: &str) -> Output {
    ...
    Command::new("node")
        ...
        .env("ADO_BUILD_ID", "1")
        .output()
        .expect("failed to spawn node scripts/ado-script/gate.js")
}
...
fn gate_js_runs_against_compiled_pipeline() {
    ...
    let gate_js = repo_root.join("scripts/ado-script/gate.js");
    if !gate_js.exists() {
