...
/// Test that the prompt-context-agent fixture compiles correctly:
/// - the new `prompt-context: true` flag is consumed (not echoed into the
///   ADO `parameters:` block)
/// - the "Prepare agent prompt" step gains a step-level `env:` mapping for
///   each prompt-context parameter
/// - the bash heredoc gains an "Additional Run Context" appender that reads
///   from `ADO_AW_CTX_*` env vars via `printf`
/// - non-context parameters (e.g. `verbose`) are NOT exposed via env vars
/// - no unreplaced `{{ ... }}` markers remain
#[test]
fn test_fixture_prompt_context_agent_compiled_output() {
    let temp_dir = std::env::temp_dir().join(format!(
        "agentic-pipeline-prompt-context-{}",
        std::process::id()
    ));
    fs::create_dir_all(&temp_dir).expect("Failed to create temp directory");
    ...
    let fixture_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures")
        .join("prompt-context-agent.md");

    let output_path = temp_dir.join("prompt-context-agent.yml");
    ...
    // ADO parameters: block should list all three params, but the ado-aw-only
    // `prompt-context` flag must not leak into it.
    assert!(
        compiled.contains("name: focusArea"),
        "Compiled output should declare focusArea parameter",
    );
    assert!(
        compiled.contains("name: relatedWorkItem"),
        "Compiled output should declare relatedWorkItem parameter",
    );
    assert!(
        compiled.contains("name: verbose"),
        "Compiled output should declare verbose parameter",
    );
    assert!(
        !compiled.contains("prompt-context:"),
        "Compiled output must not echo the ado-aw-only prompt-context flag",
    );
    assert!(
        !compiled.contains("prompt_context"),
        "Compiled output must not echo the Rust-side prompt_context field name",
    );

    // Step-level env vars: one per prompt-context param, none for verbose.
    assert!(
        compiled.contains("ADO_AW_CTX_FOCUSAREA: ${{ parameters.focusArea }}"),
        "Compiled output should map focusArea to ADO_AW_CTX_FOCUSAREA"
    );
    assert!(
        compiled.contains(
            "ADO_AW_CTX_RELATEDWORKITEM: ${{ parameters.relatedWorkItem }}"
        ),
        "Compiled output should map relatedWorkItem to ADO_AW_CTX_RELATEDWORKITEM"
    );
    assert!(
        !compiled.contains("ADO_AW_CTX_VERBOSE"),
        "Non-context parameters must not be exposed as ADO_AW_CTX_* env vars"
    );

    // Append snippet: header + one if-guarded printf per prompt-context param.
    assert!(
        compiled.contains("Additional Run Context"),
        "Compiled output should include Additional Run Context section header"
    );
    assert!(
        compiled.contains("if [ -n \"${ADO_AW_CTX_FOCUSAREA:-}\" ]; then"),
        "Compiled output should guard the focusArea append with a presence check"
    );
    assert!(
        compiled.contains("\"Focus area for this run\""),
        "Compiled output should use the displayName as the section header"
    );

    // Sanity: still no unreplaced markers anywhere.
    for line in compiled.lines() {
        let stripped = line.replace("${{", "");
        assert!(
            !stripped.contains("{{ "),
...
/// Negative test: a prompt-context parameter without a `default` must fail
/// to compile. This guarantees non-interactive triggers always have a
/// deterministic value to inject.
#[test]
fn test_prompt_context_requires_default() {
    let temp_dir = std::env::temp_dir().join(format!(
        "agentic-pipeline-prompt-context-neg-{}",
        std::process::id()
    ));
    fs::create_dir_all(&temp_dir).expect("Failed to create temp directory");

    let agent_path = temp_dir.join("bad.md");
    fs::write(
        &agent_path,
        "---\nname: bad\ndescription: bad\nparameters:\n  - name: ctx\n    type: string\n    prompt-context: true\n---\n\nbody\n",
    )
    ...
    let output_path = temp_dir.join("bad.yml");
    let binary_path = PathBuf::from(env!("CARGO_BIN_EXE_ado-aw"));
    let output = std::process::Command::new(&binary_path)
        .args([
            "compile",
    ...
    assert!(
        !output.status.success(),
        "Compiler should reject prompt-context param without default"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("default"),
        "Error should mention the missing default. stderr: {stderr}"
    );

    let _ = fs::remove_dir_all(&temp_dir);
}

/// Negative test: a prompt-context parameter whose default contains an
/// ADO expression must be rejected at compile time.
#[test]
fn test_prompt_context_rejects_ado_expression_in_default() {
    let temp_dir = std::env::temp_dir().join(format!(
        "agentic-pipeline-prompt-context-expr-{}",
        std::process::id()
    ));
    fs::create_dir_all(&temp_dir).expect("Failed to create temp directory");
    ...
    fs::write(
        &agent_path,
        "---\nname: bad\ndescription: bad\nparameters:\n  - name: ctx\n    type: string\n    default: \"leak $(System.AccessToken)\"\n    prompt-context: true\n---\n\nbody\n",
    )
    .expect("write fixture");
    ...
    assert!(
        !output.status.success(),
        "Compiler should reject ADO expression in prompt-context default"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("ADO expression"),
        "Error should mention ADO expression. stderr: {stderr}"
    );
