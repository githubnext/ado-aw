...
│           ├── trigger-e2e/ # Gate/synth-PR trigger-condition E2E harness (not a bundle; queues a victim pipeline under a battery of real trigger conditions, asserts gate decisions, files a GitHub issue on failure; `fact-catalog.gen.json` is generated by `export-fact-catalog` and used as a drift guard for the TypeScript `FACT_META` mirror)
...
- [`docs/cli.md`](docs/cli.md) — `ado-aw` CLI commands (`init`, `compile`,
  ...
  deprecated hidden alias; `export-gate-schema` and `export-fact-catalog` are
