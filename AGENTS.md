...
1. **Agent (Stage 1)** — runs the AI agent inside an AWF network-isolated
   sandbox. When configured, a trusted Azure DevOps MCP backend holds the
   Stage 1 ADO credential; the raw token is not injected into the Agent
   process. The agent produces *safe-output proposals* (e.g. "create this PR",
