...
│           ├── trigger-e2e/ # Test-only gate-spec + trigger-evaluation E2E harness (not a bundle; provides FACT_META table and gate-spec.ts; drift-guarded by export-fact-catalog → fact-catalog.gen.json)
...
│           ├── __tests__/ # Top-level TypeScript unit tests for the ado-script workspace (includes bundle-coverage.test.ts — the shipped-bundle guard)
