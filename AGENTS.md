...
│           ├── trigger-e2e/ # Test-only: FACT_META gate-spec table + trigger-evaluation E2E scenarios; drift-guarded by `export-fact-catalog` against the Rust-side `Fact` registry (not a bundle, not shipped)
