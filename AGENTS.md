...
│           ├── trigger-e2e/ # test-only: FACT_META gate-spec table (`gate-spec.ts`) + trigger-evaluation E2E scenarios; drift-guarded by `export-fact-catalog` → `fact-catalog.gen.json`; not shipped as a bundle
