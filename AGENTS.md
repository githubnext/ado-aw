...
│           ├── compiler-smoke-e2e/ # Test-only compiler-candidate smoke E2E orchestrator (not a bundle): stages the compiler under test as a `supply-chain.pipeline-artifact` source, pushes to a mirror branch, queues the registered candidate-lane pipelines, and asserts they all go green; built to `test-bin/` by `build:compiler-smoke-e2e`
