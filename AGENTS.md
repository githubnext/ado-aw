...
│           ├── compiler-smoke-e2e/ # test-only: deterministic compiler smoke E2E orchestrator (not a bundle; stages candidate compiler → pushes to mirror → queues five fixture pipelines → asserts green)
...
- [`docs/ado-script.md`](docs/ado-script.md) — `ado-script` workspace
  ...
  modelled in `src/compile/ado_bundle.rs`, the `trigger-e2e/`
  gate-spec drift guard (kept in sync via `export-fact-catalog`), and
  the `compiler-smoke-e2e/` nightly smoke orchestrator.
