...
│           ├── compiler-smoke-e2e/ # Test-only compiler smoke E2E orchestrator (not a bundle): stages a compiler candidate as a `supply-chain.pipeline-artifact` across the five safe-output fixtures, queues them on a mirror branch, and asserts they go green; see `tests/compiler-smoke-e2e/` for the fixture registry
