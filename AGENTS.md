...
│           ├── trigger-e2e/ # Trigger-condition (gate / synth-PR) E2E harness (not a bundle; validates gate decisions against real ADO builds; `export-fact-catalog` keeps its `FACT_META` in sync with the Rust `Fact` registry)
