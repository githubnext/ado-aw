...
│           ├── compiler-smoke-e2e/ # Compiler smoke E2E orchestrator (not a bundle; stages candidate compiler as a supply-chain.pipeline-artifact, pushes to a mirror branch, queues the five fixed fixture pipelines, and asserts green; referenced by `tests/compiler-smoke-e2e/`)
...
│           ├── __tests__/   # Bundle-coverage test (not a bundle; `bundle-coverage.test.ts` guards that every `src/` subdirectory is either listed in `NON_BUNDLE_DIRS` or ships a compiled bundle)
