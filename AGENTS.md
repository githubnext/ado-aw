...
│           ├── compiler-smoke-e2e/ # Compiler candidate E2E orchestrator (not a bundle): stages the candidate compiler binary from the current build as a `supply-chain.pipeline-artifact` source across the five real fixtures in `tests/safe-outputs/`, pushes to a short-lived mirror branch, queues the five "candidate lane" pipeline definitions, and asserts they all go green
