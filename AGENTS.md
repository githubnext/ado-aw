...
│           ├── compiler-smoke-e2e/ # Test-only deterministic compiler-candidate smoke E2E harness (not a bundle; built to test-bin/compiler-smoke-e2e.js by `npm run build:compiler-smoke-e2e`; exercises compiled pipeline artifacts against a real ADO project)
