...
│           ├── trigger-e2e/ # Test-only: FACT_META gate-spec table + trigger-evaluation E2E scenarios (not a bundle); drift-guarded against the Rust `Fact::ALL` registry via `export-fact-catalog` + `fact-catalog.gen.json`
