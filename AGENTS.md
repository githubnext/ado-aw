...
│           ├── trigger-e2e/ # Test-only: FACT_META gate-spec table + end-to-end trigger-evaluation scenarios (not a bundle; drift-guarded via export-fact-catalog; see docs/ado-script.md)
│           ├── __tests__/ # Bundle coverage test (bundle-coverage.test.ts; asserts every src/ subdir is either a shipped bundle or listed in NON_BUNDLE_DIRS)
