...
Detect and update Azure DevOps pipeline variables on build definitions.
...
- `--token <token>` -- token value to apply
- `--org <url>` -- Azure DevOps organization URL
- `--project <name>` -- Azure DevOps project name
- `--pat <pat>` -- PAT used for Azure DevOps API authentication
