...
- A **GitHub Personal Access Token (PAT)** -- used by the Copilot CLI at runtime
- An **Azure DevOps PAT** -- used to update the pipeline variable via the ADO API

:::caution[Temporary limitation: GitHub PAT required]
The Copilot CLI does not yet support GitHub App tokens. You must provide a **classic or fine-grained GitHub PAT** with appropriate scopes. This requirement will be removed once GitHub App token support is added to the Copilot CLI.
:::
