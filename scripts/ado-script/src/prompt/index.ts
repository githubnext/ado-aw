 ...
 * matter, runs single-pass variable substitution, appends extension
 * supplements (themselves substituted), and writes the result to the
 * configured output path. On any unrecoverable error, logs via VSO and
 * exits non-zero.
 */
...
import { stripFrontMatter, FrontMatterError } from "./frontmatter.js";
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    logError(`Failed to decode ADO_AW_PROMPT_SPEC: ${(e as Error).message}`);
    complete("Failed");
    process.exit(1);
  }
  ...
  try {
    ...
    const msg =
      e instanceof FrontMatterError ? e.message : (e as Error).message;
    logError(`Failed to strip front matter from ${spec.source_path}: ${msg}`);
    complete("Failed");
    process.exit(1);
  }

  // Assemble body + supplements first, *then* substitute in one pass.
  // Each section is individually trimmed so the joined text doesn't
  // accumulate blank lines.
  const parts: string[] = [body.trim()];
  for (const supp of spec.supplements) {
    parts.push(supp.content.trim());
  }
  const assembled = parts.filter((s) => s.length > 0).join("\n\n");

  const rendered = substitute(assembled, spec.parameters, (msg) =>
    logWarning(msg),
  );
