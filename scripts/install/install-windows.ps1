...
# Wrapped in `Install-AdoAw` and invoked on the final line so a truncated
# `iwr ... | iex` cannot execute a partial installer: PowerShell parses the
# entire script block before invocation, and an incomplete download fails
# to parse before the trailing call is reached.

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

function Install-AdoAw {
    [CmdletBinding()]
    param(
        [string]$Version,
        [string]$Repo
...
Install-AdoAw -Version $Version -Repo $Repo
