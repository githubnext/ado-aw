...
> `src/compiler-smoke-e2e/` orchestrator (not a bundle) that stages the compiler
> candidate produced by the current build (PR or nightly `main`) as a
> `supply-chain.pipeline-artifact`, pushes it to a short-lived mirror branch,
> queues the five fixed fixture pipelines listed in
> [`tests/compiler-smoke-e2e/REGISTERED.md`](../tests/compiler-smoke-e2e/REGISTERED.md),
> and asserts they all go green. It is listed in `NON_BUNDLE_DIRS` in
> `src/__tests__/bundle-coverage.test.ts` and the compiler never references it
> directly.
...
│   ├── compiler-smoke-e2e/      # test-only: compiler candidate smoke orchestrator (not a bundle); stages candidate as supply-chain.pipeline-artifact, queues five fixture pipelines on mirror branch, asserts green
│   └── __tests__/               # bundle-coverage guard: `bundle-coverage.test.ts` asserts every src/ subdir is either in NON_BUNDLE_DIRS or ships a compiled bundle
