...
│   ├── trigger-e2e/             # test-only: FACT_META gate-spec table + deterministic trigger-condition E2E harness (not a bundle)
...
│   │   ├── index.ts             # entry point: reads ADO org/project/token from env, runs scenarios, files GitHub issue on failure
│   │   ├── runner.ts            # sequential scenario runner (deterministic ordering, queues victim builds, evaluates results)
│   │   ├── queue.ts             # build-queue helpers
│   │   ├── scenario.ts          # Scenario type + helpers
│   │   ├── github-issue.ts      # GitHub issue filing for failures
│   │   ├── scenarios/           # individual trigger-condition test scenarios (bypass, pr-filters, self-cancel, synth-pr, …)
│   │   └── __tests__/           # gate-spec drift tests (gate-spec.test.ts) and runner unit tests (runner.test.ts)
