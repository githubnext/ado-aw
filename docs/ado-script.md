...
> `src/compiler-smoke-e2e/` orchestrator that stages a compiler candidate as a
> `supply-chain.pipeline-artifact` across the five real safe-output fixtures (see
> [`tests/compiler-smoke-e2e/REGISTERED.md`](../tests/compiler-smoke-e2e/REGISTERED.md)),
> pushes to a mirror branch, queues the candidate-lane pipeline definitions, and
> asserts they go green. It is **not** a runtime bundle and is **not** shipped in
> `ado-script.zip`. It is listed in `NON_BUNDLE_DIRS` in
> `src/__tests__/bundle-coverage.test.ts` so the shipped-bundle guard ignores it.
...
│   ├── compiler-smoke-e2e/      # test-only: compiler smoke E2E orchestrator (not a bundle; see tests/compiler-smoke-e2e/ for fixture registry)
