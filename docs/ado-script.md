...
> `src/compiler-smoke-e2e/` harness that drives the compiler smoke E2E suite
> (see [`tests/compiler-smoke-e2e/`](../tests/compiler-smoke-e2e/)). It
> recompiles the workflows under `tests/safe-outputs/` against the in-tree
> compiler, queues fixed ADO runs, and asserts pipeline shape. It is **not** a
> runtime bundle: it is listed in `NON_BUNDLE_DIRS` and the compiler never
...
│   ├── compiler-smoke-e2e/      # test-only: compiler smoke E2E harness (not a bundle; drives tests/compiler-smoke-e2e/)
