...
> `src/compiler-smoke-e2e/` orchestrator that stages the compiler candidate produced
> by the current build (PR or nightly `main`) as a `supply-chain.pipeline-artifact`
> source across the five real fixtures in `tests/safe-outputs/`, pushes the staged
> candidate to a short-lived branch on the mirror repo, queues the five fixed
> "candidate lane" pipeline definitions tracked in
> `tests/compiler-smoke-e2e/REGISTERED.md`, and asserts they all go green. It is
> **not** a runtime bundle: it is not part of the main `build` chain, is listed in
> `NON_BUNDLE_DIRS` in `src/__tests__/bundle-coverage.test.ts`, and the compiler
...
│   ├── compiler-smoke-e2e/      # test-only: compiler candidate E2E orchestrator (not a bundle): stages the candidate binary as a supply-chain.pipeline-artifact source across the five real fixtures in tests/safe-outputs/, queues the five "candidate lane" pipeline definitions, and asserts they all go green
