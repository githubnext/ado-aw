...
> `src/compiler-smoke-e2e/` harness that exercises compiled pipeline artifacts
> against a real ADO project (deterministic smoke E2E). It is **not** a runtime
> bundle: it is built to `test-bin/compiler-smoke-e2e.js` by
> `npm run build:compiler-smoke-e2e` (kept out of the main `build` chain and
> the release `ado-script/*.js` glob), is listed in `NON_BUNDLE_DIRS` in
> `src/__tests__/bundle-coverage.test.ts`, and the compiler never references it
> directly.
...
│   ├── compiler-smoke-e2e/      # test-only: deterministic compiler-candidate smoke E2E harness (not a bundle; built to test-bin/compiler-smoke-e2e.js)
