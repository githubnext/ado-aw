...
Used by the 1ES template as the pool **name** value (`name: {{ pool }}`).
Defaults to `AZS-1ES-L-MMS-ubuntu-22.04` when not specified.

## {{ pool_block }}

Used by non-1ES templates under `pool:` and expands to either:
- `vmImage: <image>` for Microsoft-hosted pools (default: `vmImage: ubuntu-latest`)
- `name: <pool>` for self-hosted pools
