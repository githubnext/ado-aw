...
## GitHub `create-github-issue` promotion (`0007_promote_debug_create_github_issue`)
...
to `safe-outputs.create-github-issue`, preserves other debug fields such as
...
secret continues to work through the normal Stage 3 auth path. When both the
legacy `ado-aw-debug.create-issue` and the new
