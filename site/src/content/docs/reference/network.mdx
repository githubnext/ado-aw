...
Service connections mint ADO-audience tokens for trusted pipeline roles.
Scalar permission values refer to Azure Resource Manager connections;
expanded `permissions.write` can select an Azure DevOps connection for scoped
cross-organization repository writes. Stage 3 uses
`$(System.AccessToken)` when no write override is configured.
...
- **`permissions.write`** *(optional)*: Overrides Stage 3's
  `$(System.AccessToken)` with `SC_WRITE_TOKEN`. Scalar form uses
  `connectionType: azureRM`. Expanded form supports
  `connection-type: azureDevOps` plus a deny-by-default
  organization/project/repository allow tree for cross-org repository writes.
...
# Cross-org PR, branch, or tag writes through an Azure DevOps WIF connection
permissions:
  write:
