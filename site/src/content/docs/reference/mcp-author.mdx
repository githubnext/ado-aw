...
<Aside type="caution" title="Not the SafeOutputs MCP server">
  `mcp-author` is a **local developer tool**. It is completely separate from the `ado-aw mcp` server that compiled pipelines run in Stage 1 to record safe-output proposals.
