...
job. All filter types are evaluated by the Node evaluator — there is no inline
bash codegen path.

### Gate Step Injection

Gate steps are injected into the Setup job by the target IR builders from
`Declarations::setup_steps`. The `AdoScriptExtension` Node install, bundle
download, and gate steps are emitted before user-authored setup steps.
