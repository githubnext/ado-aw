...
When any `filters:` configuration is present (and lowers to non-empty checks),
the always-on `AdoScriptExtension`
...
runtime-import resolver — see [Runtime Imports](/ado-aw/reference/runtime-imports/).
...
The gate-side steps use `setup_steps()` (not `prepare_steps()`) because
the gate must run in the **Setup job**, before the Agent job. Runtime-import
resolver steps for the agent body use `prepare_steps()` and land in the
Agent job. All filter types are evaluated by the Node evaluator — there
is no inline bash codegen path.

### Gate Step Injection

Gate steps are injected into the Setup job by `generate_setup_job()` in
`common.rs`. The `AdoScriptExtension`'s `setup_steps()` are collected
