...
<Aside type="caution">
**Literal values only.** The compiler rejects any value that contains ADO macro expressions (`$(...)`, `${{...}}`, `$[...]`) or pipeline command injection markers (`##vso[`). Values are embedded verbatim into the generated YAML — ADO variable expansion does not apply here.

To pass a pipeline variable's value to the agent, use a `setup:` step with an `env:` block on the ADO task, where ADO macro expansion is fully supported. See the [front-matter reference](/ado-aw/reference/front-matter/#custom-steps-injection) for custom step injection.
</Aside>
