...
| `model` | string | *(none)* | AI model to pass to the Copilot CLI `--model` flag. When omitted, ado-aw does not emit `--model` and the Copilot CLI chooses its own default. Any model ID supported by GitHub Copilot is accepted (e.g., `gpt-5-mini`, `claude-sonnet-5`). |
...
reads and forwards as `Authorization: Bearer <value>`. Because the token is
minted in the same job as the engine run, it resolves via a plain `$(...)` macro
— no cross-job output plumbing, no `dependsOn`.
