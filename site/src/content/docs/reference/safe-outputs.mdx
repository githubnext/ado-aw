...
The **Conclusion job** is an always-running housekeeping job emitted by the compiler whenever `safe-outputs:` is configured. It runs after SafeOutputs with `condition: always()` and is responsible for filing or appending ADO work items for diagnostic signals and pipeline failures.
