...
Temporary IDs are resolved at Stage 3 against the `create-work-item` proposals that already succeeded in the same SafeOutputs job, so they do not need `target`. A temporary ID that cannot be traced to such a create is rejected before any request is sent. When both tools are configured they must have the same effective `require-approval` setting.
