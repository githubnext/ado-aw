...
**Shallow-clone agent pools (automatic):** The diff base for the patch is computed at agent time from the checked-out repository. On agent pools whose default git fetch is shallow (`fetchDepth: 1`), a bare `checkout` leaves no `origin/<target-branch>` ref, which would otherwise prevent the diff base from being computed. Whenever `create-pull-request` is configured the compiler emits a credentialed **prepare step** that fetches and progressively deepens each repo's resolved target branch and points `origin/HEAD` at it — in the `self` checkout **and each additional `checkout:` repo dir**, so a PR to any allowed repository works. The prepare step runs in **both** the Agent job (before the agent runs, for the host-side SafeOutputs MCP diff base) and the SafeOutputs job (before `ado-aw execute`, so the Stage 3 executor's `git worktree add` resolves `origin/<target>` — each ADO job has an isolated checkout, so the ref is re-fetched in the job that builds the worktree; issue #1453). So create-pull-request works on shallow-default pools **without** forcing a full-history checkout and **without** hand-editing the compiled lock (the runtime integrity check keeps passing). Each repo's resolved `target-branch` (the PR's destination/base — see `target-branches` and `infer-target-from-checkout-ref` for per-repo targets) is deepened — not the per-repo `repos:` checkout ref. No configuration is required — see [ado-script](/ado-aw/reference/ado-script/) (`prepare-pr-base.js`).
