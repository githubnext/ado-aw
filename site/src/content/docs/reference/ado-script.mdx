...
- **`prepare-pr-base.js`** — Agent `patch-base` mode uses ADO diff metadata to fetch and verify only the source/target history needed for the merge-base (bounded 200/500/2000 dual-ref fallback); SafeOutputs `target-worktree` mode fetches only the target tip at depth 1 (issue #1453)
...
1. **Synthetic merge commit** — ADO's default PR checkout produces a merge
   ...
   in a shallow checkout, it fetches both target and source refs with bounded
   deepening and retries before failing closed.
2. **Bounded deepening** — when `HEAD` is a normal commit (shallow clone
   without a merge commit), the bundle fetches source and target together at
   increasing depths (`--depth=200`, `500`, `2000`) until
