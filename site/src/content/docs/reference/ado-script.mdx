...
- **`prepare-pr-base.js`** — fetches/deepens the `create-pull-request` target branch in the `self` checkout and every additional `checkout:` repo dir, so the SafeOutputs MCP server can compute a diff base for a PR to any allowed repo on shallow-default agent pools (Agent job)
