...
| `filters` | No | Runtime gate filters evaluated inside the Setup job (see [Filter IR](/ado-aw/reference/filter-ir/)) |
...
See the [Filter IR reference](/ado-aw/reference/filter-ir/) for the full
catalog of runtime filter types, validation rules, and compilation details.
