...
The `on:` field is the **complete declaration** of when a pipeline runs. Four
sub-fields are supported — `push`, `schedule`, `pipeline`, and `pr` — and any
combination of them is valid. If a workflow has no `on:` key at all, it
compiles to a manual / API-queued-only pipeline: Azure DevOps otherwise reads
a **missing** top-level `trigger:` key as *"run CI on every branch"*, not
*"no CI"*, so the compiler always emits an explicit `trigger:` (and `pr:`)
instead of relying on that implicit default.

### `on.push` — push (CI) trigger

Maps directly onto ADO's native `trigger:` key.
...
**An explicit `on.push` always wins**, including over the all-branches
trigger that `on.pr`'s default `mode: synthetic` emits, and over the `none`
that `on.schedule` or `on.pr.mode: policy` would otherwise produce. It
controls only the top-level `trigger:` key — `on.pr` independently drives
the `pr:` key, so setting `on.push` never enables or disables PR triggering.

`on.push` is unrelated to
[`execution-context.ci-push`](/ado-aw/reference/execution-context/), which
stages context facts *inside* a build that has already started.
...
|--------|---------------|----------|
| `synthetic` (default) | all branches (`include: ['*']`) | No Build Validation branch policy is installed. Setup synthesizes PR context from the ADO API. **Recommended for most agents.** |
| `policy` | `trigger: none` | Operator has installed an ADO Build Validation branch policy. Prevents duplicate CI builds. |

An explicit `on.push` overrides the `trigger:` column in both rows.
...
**`mode: synthetic` (default)** — fires the pipeline on every push (all-branches ADO `trigger:`, unless overridden by an explicit `on.push`). A Setup-job step calls the ADO REST API to find an open PR for the pushed branch:
...
regardless of whether `schedule` or `pipeline` is also set. An explicit
`on.push` always overrides the CI (push) column below, in every row.

| `on.schedule` | `on.pipeline` | `on.pr` | CI (push) | PR events |
|:---:|:---:|:---:|---|---|
| — | — | — | `none` (manual / API-queued only) | `none` |
| ✓ | — | — | `none` | `none` |
| — | ✓ | — | `none` | `none` |
| ✓ | ✓ | — | `none` | `none` |
| — | — | ✓ | all branches (synthetic mode) | ✓ enabled |
| ✓ | — | ✓ | all branches (synthetic mode) | ✓ enabled |
| — | ✓ | ✓ | all branches (synthetic mode) | ✓ enabled |
