...
|---|---|---|
| `-o, --output <dir>` | `${TEMP}/ado-aw/audit` | Directory under which `<dir>/build-<id>/` is written. Shared with `ado-aw trace` and the mcp-author tools so concurrent invocations reuse each other's downloads. |
