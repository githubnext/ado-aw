...
Use scalar `permissions.write` for an Azure Resource Manager connection:
...
For cross-organization repository writes, use an Azure DevOps WIF connection
and explicit scope:
...
principal. The expanded Azure DevOps form is the supported option for
**cross-org repository** writes.
...
|---|---|---|
| 401 | `TF400813: ... is not authorized` | Token mint failed (check the AzureCLI@3 step), connection type is wrong, or token is malformed |
