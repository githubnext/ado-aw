...
need a named write identity, cross-project access, or scoped cross-org
repository writes, see the
...
The `comment-on-work-item` tool requires an explicit `target:` field. Compilation fails if it is missing.
...
### "safe-outputs.update-work-item requires a 'target' field"

Like `comment-on-work-item`, the `update-work-item` tool requires an explicit `target:` field. Compilation fails if it is missing.

**Solution:** Add a `target:` field and at least one enabled field:

```yaml
safe-outputs:
  update-work-item:
    title: true
    target: "*"   # any work item, or a specific work item ID number (e.g. 42)
```
...
See the dedicated [Safe-output 401/403 errors](/ado-aw/troubleshooting/safe-output-permissions/) page for the full diagnosis flow, a permission bitmask decoder, a REST recipe for inspecting the ACEs, and the three fix paths (write service connection / flip the auth-scope toggle / lift the Deny).
