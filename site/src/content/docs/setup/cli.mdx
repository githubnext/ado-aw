...
The `ado-aw` CLI provides commands for authoring, compiling, and managing the full lifecycle of Azure DevOps agentic workflows.
...
Initialize a repository for AI-first agentic workflow authoring. Creates `.github/agents/ado-aw.agent.md`, a Copilot dispatcher agent that routes to the create, update, and debug prompts.
...
Compile a markdown agent file into Azure DevOps pipeline YAML. If you omit the path, `ado-aw` auto-discovers and recompiles all agentic workflow sources in the current directory.
