...
1. Start with built-in capabilities such as SafeOutputs — test whether the agent works with only GitHub MCP and SafeOutputs tools.

2. Add `tools.azure-devops` if the agent needs Azure DevOps tooling — configure toolsets and allowed tools for ADO operations.

3. Add custom `mcp-servers:` only for extra capabilities — use stdio containers or HTTP backends for specialized tools.

4. Keep `allowed:` lists tight — only expose the specific tools the agent genuinely needs.

5. Use `env:` passthrough for secrets and tokens instead of hardcoding values — keep credentials out of the agent file with blank-string env mappings.
