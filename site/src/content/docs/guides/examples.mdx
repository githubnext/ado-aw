...
:::tip[repos: shorthand]
This example predates the `repos:` shorthand and still uses the legacy `repositories:` form. Both are accepted, and the legacy form is automatically rewritten to `repos:` on the next `ado-aw compile`. For new agents, prefer the shorter syntax:
...
An example that files GitHub issues from an Azure DevOps pipeline run.

**Key features:**
- `create-github-issue` safe output (GitHub issues, distinct from ADO `create-work-item`)
- Target repository configuration
- Label and assignee policies
- Per-run budget control (`max: 3`)

:::note[Stage 3 GitHub credential]
GitHub issue outputs call GitHub only from Stage 3, after threat detection. The
write credential (`ADO_AW_GITHUB_TOKEN` or a GitHub App) is never exposed to the
agent. See the [safe outputs reference](/ado-aw/reference/safe-outputs/).
:::

**Use this when:** You want an agent to report pipeline failures as GitHub issues in a repository you control.
